...
const regex = /background:\s+var\(--aero-glass-bg\);\s+backdrop-filter:\s+var\(--aero-blur\);\s+-webkit-backdrop-filter:\s+var\(--aero-blur\);\s+border:\s+1px\s+solid\s+var\(--aero-glass-border\);\s+border-radius:\s+var\(--aero-radius\);\s+box-shadow:\s+var\(--aero-shadow\);/g;
